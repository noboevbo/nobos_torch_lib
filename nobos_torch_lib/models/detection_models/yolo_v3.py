...
def convert2cpu(torch_data: torch.Tensor):
    if torch_data.is_cuda:
        return torch.FloatTensor(torch_data.size()).copy_(torch_data)
    else:
        return torch_data


def parse_cfg(model_state_file: str):
    """
...
class DetectionLayer(nn.Module):
    ...
    def forward(self, x, inp_dim, num_classes, confidence):
        x = x.data
        prediction = x
        prediction = predict_transform(prediction, inp_dim, self.anchors, num_classes)
        return prediction
...
class Darknet(nn.Module):
    ...
    def load_weights(self, model_state_file: str):

        # Open the weights file
        fp = open(model_state_file, "rb")
        ...
        for i in range(len(self.module_list)):
            ...
            if module_type == "convolutional":
                ...
                if (batch_normalize):
                    ...
                    # Copy the nobos_commons.data_structures to model
                    ...
                    # Finally copy the nobos_commons.data_structures
