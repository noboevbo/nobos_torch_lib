...
def bb_to_center_scale(bb: BoundingBox):
    aspect_ratio = 0.75 # input_img_width to Input_img_height ratio
    pixel_std = 200 # Standard bb size
    center = np.zeros((2), dtype=np.float32)
    center[0] = bb.top_left.x + bb.width * 0.5
    center[1] = bb.top_left.y + bb.height * 0.5
...
def predict_transform(prediction, inp_dim, anchors, num_classes, use_gpu: bool):
...
def write_results(prediction, confidence, num_classes, nms=True, nms_thresh=0.4):
    ...
    for ind in range(batch_size):
        ...
        for cls in img_classes:
            ...
            # Concatenate the batch_id of the image to the detections
            # this helps us identify which image does the detections correspond to
